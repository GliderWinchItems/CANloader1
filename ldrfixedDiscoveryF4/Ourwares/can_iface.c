...
08/26/2022 - Mods to work with GliderWinch/Items/CANloader (no FreeRTOS)
...
#ifdef CHEATINGONHAL
#include "stm32l4xx.h" 	// **** CHEATING (processor dependent) ****
#endif
 ...
 * @return  : 0 = CAN msg not for us; 1 = CAN msg is in mbx list
 * *************************************************************************/
static uint8_t canmsg_compress(struct CANRCVBUF *pcan, CAN_RxHeaderTypeDef *phal, uint8_t *pdat)
{
	if (phal->IDE != 0)
	{ // Extended 29b CAN id
	...
	/* Sofware filter incoming CAN IDs. */

	/*  I_AM_CANID is passed in from command line in script
	    'mm' via 'make' with C_DEFS -D in Makefile. 
...
//				taskEXIT_CRITICAL();
	...
	*ppctllist = pctl;	
	ppctllist++;
	if (ppctllist == &pctllist[MAXCANMODULES]) 
	{ 
//		taskEXIT_CRITICAL();
		return NULL;//JIC too many entries
	} 
