...
uint32_t dbugabort[32];
#define DBUGXSZ 64
struct DBUGX
{
	uint32_t idb4;  // id before MBX load
	uint32_t idaf;  // id after MBX loaded
 	uint16_t aflag; // abort flag
	int16_t updn;  // loadmbx2+ interrupt - ctr
	uint32_t ct1;   // Num aborts handled
	uint32_t mbx2;  // Source of loadmbx2
   uint32_t ct;    // Number msgs pending in linked list
	uint32_t arbq;  // Ct of arbitration requests
	uint32_t bit;   // TME0
} dbugx[DBUGXSZ];
uint32_t dbugxidx = 0;
uint32_t arbqctr = 0;
...
static void loadmbx2(struct CAN_CTLBLOCK* pctl, uint16_t dbug);
...
extern uint32_t	pclk1_freq;	/*	SYSCLKX/PCLK1DIV	E.g. 72000000 	*/
...
static struct CAN_CTLBLOCK* pctl1;	// Pointer to control block for CAN2
 uint32_t buffct = 0;	// Number of msg blocks buffered (total)
 int friict = 0;	// Number of msg blocks on the free list

uint32_t abortflag = 0;	// 1 = ABRQ0 bit in TSR was set.

/*---------------------------------------------------------------------------------------------
 * static void disable_ints(uint32_t* p[2]);
 * @brief	: disable CAN1 and CAN2 TX, RX0, RX1 interrupts
 * @param	: p[2] = pointer to save
 ----------------------------------------------------------------------------------------------*/
static void disable_ints(uint32_t p[])
{
...
/*---------------------------------------------------------------------------------------------
 * static void disable_TXints(uint32_t* p[2]);
 * @brief	: disable CAN1 and CAN2 TX interrupts;' leave RX interrupts alone
 * @param	: p[2] = pointer to save
 ----------------------------------------------------------------------------------------------*/
static void disable_TXints(uint32_t p[])
{
...
/*---------------------------------------------------------------------------------------------
 * static void reenable_ints(uint32_t* p[2]);
 ...
 * @param	: p2 = pointer to CAN2 IER before disable
 ----------------------------------------------------------------------------------------------*/
static void reenable_ints(uint32_t p[])
{
	CAN_IER(CAN1) |= p[0];
	CAN_IER(CAN2) |= p[1];
	return;
...
/*---------------------------------------------------------------------------------------------
 * static u8 bitcvt(uint8_t bit);
 * @brief	: convert a non-zero byte into a 1
 * @param	: input bit
 * @return	: bit
 ----------------------------------------------------------------------------------------------*/
static uint8_t bitcvt(u8 bit)
{
	if (bit != 0) return 1;
	return 0;
}
...
//	pctl->ptrs1.ptail = NULL; // RX1
	...
	uint32_t brp = (pclk1_freq / (1 + p->tbs1 + p->tbs2) ) / p->baudrate;
...
int can_driver_put(struct CAN_CTLBLOCK* pctl, struct CANRCVBUF *pcan, u8 maxretryct, u8 bits)
{
	...
	uint32_t save[2];	// IER register saved during disable of interrupts
...
/******************************************************************************
 * uint32_t can_driver_getcount(struct CAN_CTLBLOCK* pctla, struct CAN_CTLBLOCK* pctlb, struct CAN_BLOCK_CTS* pcts);
...
dbugabort[17] += 1;
	...
	uint32_t tsr = CAN_TSR(pctl->vcan);	// Copy of status register mailbox 0 bits
...
struct RXIRQPARAM	// For passing constants to RX IRQ Handler
{
	...
	uint32_t fifo;			// CAN_FIFO0|CAN_FIFO1
	uint32_t rfr;			// CAN_RFR0|CAN_RFR1
};
volatile uint32_t debugT1;
