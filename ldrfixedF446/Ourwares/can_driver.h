...
struct CAN_NVIC 
{
	uint32_t	vectnum;	// Interrupt vector number
	uint32_t 	vectpriority;	// Interrupt priority
};
struct CAN_PARAMS2
{
	uint32_t	baudrate;	// E.g. 500000 = 500K bps
	uint8_t	cannum;		// These params apply to-- 0 = CAN1, 1 = CAN2
	uint8_t	port;		// Port code:  CAN1 port: 0 = PA 11|12; 1 = PB 8|9; 2 = PD 0|1;  (port > 3 not valid) 
				//             CAN2 port: 3 = PB 5|6; 4 = PB 12|13 ;  (port > 5 not valid) 
	uint8_t	silm;		// CAN_BTR[31] Silent mode (0 or non-zero)
	uint8_t	lbkm;		// CAN_BTR[30] Loopback mode (0 = normal, non-zero = loopback)
	uint8_t	sjw;		// CAN_BTR[24:25] Resynchronization jump width
	uint8_t	tbs2;		// CAN_BTR[22:20] Time segment 2 (e.g. 5)
	uint8_t	tbs1;		// CAN_BTR[19:16] Time segment 1 (e.g. 12)
	uint8_t	dbf;		// CAN_MCR[16] Debug Freeze; 0 = normal; non-zero =
	uint8_t	ttcm;		// CAN_MCR[7] Time triggered communication mode
	uint8_t	abom;		// CAN_MCR[6] Automatic bus-off management
	uint8_t	awum;		// CAN_MCR[5] Auto WakeUp Mode
	uint8_t	nart;		// CAN_MCR[4] No Automatic ReTry (0 = retry; non-zero = auto retry)
...
struct CAN_CTLBLOCK
{
	uint32_t	vcan;			// CAN1 or CAN2 base address
...
struct CAN_INIT
{
	uint32_t	numbuff;	// TOTAL (both CAN1&2) ct buffered (1)
